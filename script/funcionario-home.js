...
if (!funcionario || !funcionario.func_id) window.location.href = "./login-funcionario.html";
...
async function carregarOpcoesTreino() {
  try {
    const [treinosResposta, exerciciosResposta] = await Promise.all([
      fetch(`${api}/treinos?func_id=${funcionario.func_id}`),
...
async function renderTreinos() {
  const resposta = await fetch(`${api}/treinos?func_id=${funcionario.func_id}`);
